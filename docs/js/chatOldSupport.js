...
async function checkAPIStatus() {
  ...
  try {
      const serversNotDown = await fetchWithXHR('https://api.botbridge.net/api/servers-not-down');
      const serversListed = await fetchWithXHR('https://api.botbridge.net/api/servers-listed');
      ...
      updateStatus(statusTextElement, operationalPercentage, totalServers);
  } catch (error) {
...
function updateStatus(statusTextElement, operationalPercentage, totalServers) {
  if (totalServers === 0) {
